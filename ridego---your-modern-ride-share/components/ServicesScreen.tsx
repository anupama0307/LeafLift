...
const ServicesScreen: React.FC = () => {
  const ServiceCard: React.FC<{ item: ServiceItem }> = ({ item }) => (
    <div className="flex flex-col items-center gap-1.5 cursor-pointer hover:scale-105 transition-transform group">
      <div className="relative bg-[#f3f3f3] dark:bg-zinc-800 w-full aspect-square rounded-2xl flex items-center justify-center overflow-hidden">
        ...
        {item.promo && (
          <div className="absolute top-2 left-2 bg-black dark:bg-[#f2b90d] text-white dark:text-black text-[9px] font-black px-1.5 py-0.5 rounded-full uppercase tracking-tighter">
            {item.promo}
          </div>
        )}
      </div>
  ...
  return (
    <div className="px-5 pb-24 pt-4 animate-in fade-in duration-500 bg-white dark:bg-[#121212]">
      ...
      <div className="mt-12 bg-black text-white p-6 rounded-3xl relative overflow-hidden">
        <div className="relative z-10">
          <h3 className="text-xl font-bold mb-2">Save with UPI</h3>
          <p className="text-sm text-zinc-400 max-w-[180px]">Get ₹50 off on your next 3 Moto rides.</p>
        </div>
        <div className="absolute right-0 bottom-0 top-0 w-1/3 flex items-center justify-center">
            <span className="material-icons-outlined text-8xl opacity-10 -rotate-12 text-[#f2b90d]">motorcycle</span>
        </div>
      </div>
    </div>
  );
